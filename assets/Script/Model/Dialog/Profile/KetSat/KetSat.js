...
cc.Class({
    ...
    onClickAC:  function(){
    	...
    	if (this.isGui) {
    		...
    		if (data.gui < 10000) {
    			cc.RedT.inGame.notice.show({title: "GỬI KUN", text: "Số tiền gửi phải lớn hơn 10.000"});
    			return void 0;
    		}
    	}else{
    		data.rut = {red: helper.getOnlyNumberInString(this.inputRut.string), otp: this.inputOTP.string};
    		if (data.rut < 10000) {
    			cc.RedT.inGame.notice.show({title: "RÚT KUN", text: "Số tiền rút phải lớn hơn 10.000"});
    			return void 0;
    		}
    	}
